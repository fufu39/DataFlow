...
const getTooltipHtml = (d) => {
  ...
  return `
    ...
    <div class="tooltip-content">
      <div>1. 节点ID：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${d.nodeId}</div>
      <div>2. 节点身份：&nbsp;${identity}</div>
      <div>3. 节点IP：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${d.mockIp}</div>
      <div>4. 流转时间：&nbsp;${d.mockTime}</div>
    </div>
  `
}
...
const nodes = ref([
  // Layer 0: 源节点
  { id: 'source-1', name: '源节点', nodeId: 'AAU01', layer: 0, type: 'source' },

  // Layer 1: 平台节点
  { id: 'platform-1', name: '平台节点', nodeId: 'CCU33', layer: 1, type: 'platform' },
  { id: 'platform-2', name: '平台节点', nodeId: 'HKL05', layer: 1, type: 'platform' },
  { id: 'platform-3', name: '平台节点', nodeId: 'FBT95', layer: 1, type: 'platform' },

  // Layer 2: 中间节点
  { id: 'intermediate-1', name: '中间节点', nodeId: 'VYU13', layer: 2, type: 'intermediate' },
  { id: 'intermediate-2', name: '中间节点', nodeId: 'XYU14', layer: 2, type: 'intermediate' },
  { id: 'intermediate-3', name: '中间节点', nodeId: 'ZYU15', layer: 2, type: 'intermediate' },

  // Layer 3: 目的节点
  { id: 'dest-1', name: '目的节点', nodeId: 'BKM08', layer: 3, type: 'destination' },
  { id: 'dest-2', name: '目的节点', nodeId: 'AZU17', layer: 3, type: 'destination' },
  { id: 'dest-3', name: '目的节点', nodeId: 'RTY32', layer: 3, type: 'destination' }
])
// 为每个节点附加固定的模拟IP和时间
nodes.value.forEach((node) => {
  node.mockIp = `${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(
    Math.random() * 255
...
const links = ref([
  ...
  { source: 'source-1', target: 'platform-2' },
  { source: 'source-1', target: 'platform-3' },

  // 平台节点到中间节点
  { source: 'platform-1', target: 'intermediate-1' },
  { source: 'platform-2', target: 'intermediate-2' },
  { source: 'platform-3', target: 'intermediate-3' },

  // 中间节点到目的节点
  { source: 'intermediate-1', target: 'dest-1' },
  { source: 'intermediate-2', target: 'dest-2' },
  { source: 'intermediate-3', target: 'dest-3' }
])
...
onMounted(() => {
  dataInfo.value = {
    ...
    dataLength: route.query.dataLength,
    dataType: route.query.dataType,
    sensitivityLevel: route.query.sensitivityLevel,
    creationTime: route.query.creationTime.slice(0, 10)
  }
...
const initChart = (svgElement) => {
  ...
  const layerWidth = width / (layers.length + 3)
  ...
  if (bounds.width > 0 && bounds.height > 0) {
    ...
    const translateX = fullWidth / 2 - scale * midX + 60 // 向左偏移一点
    ...
    <el-card>
      <template #header>
        <el-page-header :icon="ArrowLeft" @back="goBack">
          <template #content>
            <span class="page-header-title"> 数据溯源链条详情 </span>
          </template>
        </el-page-header>
      </template>
        ...
        <div class="user-info-panel">
          <div class="panel-header">
            <div class="avatar-pulse">
              ...
              >
                <line x1="18" y1="20" x2="18" y2="11"></line>
          ...
          <div class="panel-body">
            <div class="info-row">
              ...
              <span class="info-value user-name">{{ dataInfo.dataId || '...' }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">源数据长度</span>
              <span class="info-value">{{ dataInfo.dataLength || '...' }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">数据类型</span>
              <span class="info-value">{{ dataInfo.dataType || '...' }}</span>
            </div>
            <div class="info-row">
            ...
            <div class="info-row">
              <span class="info-label">产生时间</span>
              <span class="info-value">{{ dataInfo.creationTime || '...' }}</span>
            </div>
          </div>
        </div>
        <!-- 提示框 -->
